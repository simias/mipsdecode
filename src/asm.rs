...
fn op_j(instruction: Instruction) -> String {
    ...
    format!("j (PC & 0xf0000000) | 0x{:x}", i << 2)
}

fn op_jal(instruction: Instruction) -> String {
    let i = instruction.imm_jump();

    format!("jal (PC & 0xf0000000) | 0x{:x}", i << 2)
}
...
fn op_swl(instruction: Instruction) -> String {
    let i = instruction.imm_se();
    let t = instruction.t();
    let s = instruction.s();

    format!("swl {}, [{} + 0x{:x}]", reg(t), reg(s), i)
}
...
fn op_swr(instruction: Instruction) -> String {
    let i = instruction.imm_se();
    let t = instruction.t();
    let s = instruction.s();
